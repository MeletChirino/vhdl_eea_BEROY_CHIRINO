...
void set_freq(int frequency){
	...
	IOWR_ALTERA_AVALON_PIO_DATA(0x811250, frequency);
}

void set_duty(int duty){
	//mira si lo puedes poner ingresar un numero entre 0 y 100 y hacerle el map desde 0 hasta el valor de la frequencia
	//*DUTY_BASE = duty;
	IOWR_ALTERA_AVALON_PIO_DATA(0x811240, duty);
}

int main()
{
	int angle_barre = 0;//0x21090
	alt_putstr("Hello from Nios II!\n");
	set_freq(0xf0ff);
	set_duty(0x4f40);
	while(1){
		for(int i=0; i<20; i++)
			angle_barre = IORD_ALTERA_AVALON_PIO_DATA(ANGLE_BARRE_BASE);
	};
